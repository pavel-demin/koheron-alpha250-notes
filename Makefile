...
CORES = axi_hub axis_adc axis_constant axis_controller axis_counter axis_fifo \
  axis_spi axis_variable dsp48 port_slicer
...
$(UBOOT_DIR): $(UBOOT_TAR)
	...
	cp patches/zynq_koheron_alpha250_defconfig $@/configs
	cp patches/zynq-koheron-alpha250.dts $@/arch/arm/dts
...
$(UBOOT_DIR)/u-boot.bin: $(UBOOT_DIR)
	mkdir -p $(@D)
	make -C $< mrproper
	make -C $< ARCH=arm zynq_koheron_alpha250_defconfig
	make -C $< ARCH=arm -j $(shell nproc 2> /dev/null || echo 1) \
